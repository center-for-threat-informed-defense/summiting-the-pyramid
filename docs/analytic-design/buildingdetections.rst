...
actually occurring while remaining difficult for an adversary to evade. Building detections this way requires thoughtful decisions about the signals
...
may also occur frequently during legitimate activity. This creates an important design trade-off. Detection engineers should seek
...
and environment in which the analytic will run. See :ref:`Detection Quality <detection-quality>` for detailed guidance on
...
easily an adversary can evade it. Highly specific indicators such as hashes, filenames, process names, or
...
disappear with relatively inexpensive changes. For detections intended to provide durable behavioral coverage, favor
...
contextual signals may be necessary. A useful signal therefore balances two questions:
...
make the resulting detection meaningful. The :ref:`Detection Decomposition Diagram (D3)
...
necessarily the same as detecting the behavior that tool can perform. An analytic may identify a particular executable, service name, command
...
observable has. For detections intended to provide durable behavioral coverage, ask:
...
identity of a particular tool. Tool- and indicator-based detections still have important uses, particularly
...
observing activity that *could be used* to perform adversary behavior. Many legitimate tools and system functions can also be used maliciously.
...
claiming detection coverage. When evaluating detection logic, ask:
...
necessary. Inference cannot always be eliminated. Some behaviors are inherently
...
additional evidence where necessary. See :ref:`Using Context to Determine Intent
...
A detection must work in the environment in which it is deployed. Behavior that is unusual in one organization may be routine in another.
...
distinguishing malicious from legitimate activity. Testing against representative environmental data can help identify recurring
...
can become an evasion opportunity. When designing exclusions:
...
observable or analytic. In these situations, multiple observations may collectively provide the
...
information that the individual analytics cannot provide independently. See :ref:`Chaining Analytics <Chaining Analytics>` for guidance on combining
...
detects. This is particularly important when mapping detection content to ATT&CK.
...
behavior. See :ref:`Mapping Detections to ATT&CK <mapping-detections-to-attack>` for
...
one-time queries. Adversary behavior evolves, ATT&CK changes, telemetry sources change, and
...
implementations. The :ref:`Detection Engineering Workflow <detection-engineering-workflow>`
